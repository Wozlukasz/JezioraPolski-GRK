...
int main() {
  ...
  GLFWwindow *window =
      glfwCreateWindow(1024, 768, "Lake Strzeszynek 3D Viewer", NULL, NULL);
  if (!window) {
    glfwTerminate();
    return -1;
  }
  ...
  // Shadery
  unsigned int terrainShader = createShaderProgramFromFiles(
      "Assets/shaders/terrain.vert", "Assets/shaders/terrain.frag");
  unsigned int waterShader = createShaderProgramFromFiles(
      "Assets/shaders/water.vert", "Assets/shaders/water.frag");
  unsigned int skyboxShader = createShaderProgramFromFiles(
      "Assets/shaders/skybox.vert", "Assets/shaders/skybox.frag");
  unsigned int shadowShader = createShaderProgramFromFiles(
      "Assets/shaders/shadow.vert", "Assets/shaders/shadow.frag");
  unsigned int plantShader =
      createShaderProgramFromFiles("Assets/shaders/instanced_plant.vert",
                                   "Assets/shaders/instanced_plant.frag");
  unsigned int shadowPlantShader =
      createShaderProgramFromFiles("Assets/shaders/shadow_instanced.vert",
                                   "Assets/shaders/shadow_instanced.frag");
  unsigned int fishShader = createShaderProgramFromFiles(
      "Assets/shaders/fish.vert", "Assets/shaders/fish.frag");
  unsigned int fishShadowShader = createShaderProgramFromFiles(
      "Assets/shaders/fish_shadow.vert", "Assets/shaders/fish_shadow.frag");
  unsigned int bubbleShader = createShaderProgramFromFiles(
      "Assets/shaders/bubble.vert", "Assets/shaders/bubble.frag");
  ...
  loadOBJ(findAssetPath("Assets/models/strzeszynek/teren-jezioro.obj"),
          globalTerrainVertices);
  loadOBJ(findAssetPath("Assets/models/strzeszynek/woda-jezioro.obj"),
          waterVertices);
  ...
  glBufferData(GL_ARRAY_BUFFER, globalTerrainVertices.size() * sizeof(Vertex),
               globalTerrainVertices.data(), GL_STATIC_DRAW);
  ...
  unsigned int splatMapTex = loadTexture(
      findAssetPath("Assets/materials/strzeszynek/strzeszynek-splat-map.png")
          .c_str());
  unsigned int mudTex = loadTexture(
      findAssetPath(
          "Assets/materials/strzeszynek/teren-jezioro/mud/mud diffuse.bmp")
          .c_str());
  unsigned int soilTex = loadTexture(
      findAssetPath(
          "Assets/materials/strzeszynek/teren-jezioro/soil/soil-22-diffuse.jpg")
          .c_str());
  unsigned int grassTex =
      loadTexture(findAssetPath("Assets/materials/strzeszynek/teren-jezioro/"
                                "grass/PX_Ground_Grass_02_albedo.jpg")
                      .c_str());
  unsigned int waterTex = loadTexture(
      findAssetPath(
          "Assets/materials/strzeszynek/woda-jezioro/woda-jezioro-diffuse.png")
          .c_str());
  unsigned int mudNorm = loadTexture(
      findAssetPath(
          "Assets/materials/strzeszynek/teren-jezioro/mud/mud normal.bmp")
          .c_str());
  unsigned int soilNorm =
      loadTexture(findAssetPath("Assets/materials/strzeszynek/teren-jezioro/"
                                "soil/soil-22-diffuse_normal.jpg")
                      .c_str());
  unsigned int grassNorm =
      loadTexture(findAssetPath("Assets/materials/strzeszynek/teren-jezioro/"
                                "grass/PX_Ground_Grass_02_normal.jpg")
                      .c_str());
  unsigned int mudRough = loadTexture(
      findAssetPath(
          "Assets/materials/strzeszynek/teren-jezioro/mud/mud roughness.bmp")
          .c_str());
  unsigned int grassRough =
      loadTexture(findAssetPath("Assets/materials/strzeszynek/teren-jezioro/"
                                "grass/PX_Ground_Grass_02_roughness.jpg")
                      .c_str());
  ...
  unsigned int underwaterCubemap =
      generateUnderwaterCubemap(skyboxShader, skyboxVAO, 512);

  // ---- Cache uniform locations (avoid per-frame string lookups) ----
  GLint loc_shadow_lightSpaceMatrix =
      glGetUniformLocation(shadowShader, "lightSpaceMatrix");
  GLint loc_shadow_model = glGetUniformLocation(shadowShader, "model");
  GLint loc_shadowPlant_lightSpaceMatrix =
      glGetUniformLocation(shadowPlantShader, "lightSpaceMatrix");
  ...
  GLint loc_terrain_projection =
      glGetUniformLocation(terrainShader, "projection");
  GLint loc_terrain_lightSpaceMatrix =
      glGetUniformLocation(terrainShader, "lightSpaceMatrix");
  ...
  GLint loc_terrain_flashlightOn =
      glGetUniformLocation(terrainShader, "flashlightOn");
  GLint loc_terrain_flashlightPos =
      glGetUniformLocation(terrainShader, "flashlightPos");
  GLint loc_terrain_flashlightDir =
      glGetUniformLocation(terrainShader, "flashlightDir");
  ...
  GLint loc_plant_lightSpaceMatrix =
      glGetUniformLocation(plantShader, "lightSpaceMatrix");
  ...
  GLint loc_plant_flashlightOn =
      glGetUniformLocation(plantShader, "flashlightOn");
  GLint loc_plant_flashlightPos =
      glGetUniformLocation(plantShader, "flashlightPos");
  GLint loc_plant_flashlightDir =
      glGetUniformLocation(plantShader, "flashlightDir");
  ...
  GLint loc_skybox_projection =
      glGetUniformLocation(skyboxShader, "projection");
  ...
  GLint loc_skybox_isReflectionPass = glGetUniformLocation(skyboxShader, "isReflectionPass");
  GLint loc_terrain_isReflectionPass = glGetUniformLocation(terrainShader, "isReflectionPass");
  GLint loc_plant_isReflectionPass = glGetUniformLocation(plantShader, "isReflectionPass");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // Oblicz macierze kamery wcześniej — potrzebne do frustum culling w obu
    // passach
    int width, height;
    glfwGetFramebufferSize(window, &width, &height);
    glm::mat4 view = glm::translate(
        glm::mat4_cast(glm::conjugate(cameraOrientation)), -cameraPos);
    float aspect = (height == 0) ? 1.0f : (float)width / (float)height;
    glm::mat4 projection =
        glm::perspective(glm::radians(fov), aspect, 0.1f, 2000.0f);
    glm::mat4 vpMatrix = projection * view;

    glm::mat4 lightProjection =
        glm::ortho(-400.0f, 400.0f, -400.0f, 400.0f, 1.0f, 1000.0f);
    glm::vec3 lightPos = cameraPos + normalize(lightDir) * 300.0f;
    glm::mat4 lightView =
        glm::lookAt(lightPos, cameraPos, glm::vec3(0.0, 1.0, 0.0));
    ...
    if (cameraPos.y > 63.0f) { // Renderuj odbicie tylko gdy jesteśmy nad wodą
        ...
        glm::mat4 reflectMat = glm::mat4(1.0f);
        reflectMat = glm::translate(reflectMat, glm::vec3(0.0f, 64.0f, 0.0f));
        reflectMat = glm::scale(reflectMat, glm::vec3(1.0f, -1.0f, 1.0f));
        reflectMat = glm::translate(reflectMat, glm::vec3(0.0f, -64.0f, 0.0f));
        
        glm::mat4 refView = view * reflectMat;
        
        // Odwracamy winding order, bo skala -1 obraca trójkąty na lewą stronę
        glFrontFace(GL_CW);

        glDepthFunc(GL_LEQUAL);
        glUseProgram(skyboxShader);
        // Niebo renderujemy po prostu z odbitym widokiem (bez translacji z refView)
        glm::mat4 skyRefView = glm::mat4(glm::mat3(refView));
        glUniformMatrix4fv(loc_skybox_view, 1, GL_FALSE, glm::value_ptr(skyRefView));
        glUniformMatrix4fv(loc_skybox_projection, 1, GL_FALSE, glm::value_ptr(projection));
        glm::vec3 refCamPos = glm::vec3(cameraPos.x, 64.0f - (cameraPos.y - 64.0f), cameraPos.z);
        glUniform3fv(loc_skybox_viewPos, 1, glm::value_ptr(refCamPos));
        glUniform1i(loc_skybox_useCubemap, 0);
        glUniform1i(loc_skybox_isReflectionPass, 1);
        glBindVertexArray(skyboxVAO);
        ...
        glUniform1i(loc_terrain_clipMode, 1);
        glUniform1i(loc_terrain_isReflectionPass, 1);
        ...
        glUniform1i(loc_plant_clipMode, 1);
        glUniform1i(loc_plant_isReflectionPass, 1);
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    glUseProgram(terrainShader); 
    glUniform1i(loc_terrain_clipMode, 0);
    glUniform1i(loc_terrain_isReflectionPass, 0);
    glUseProgram(plantShader); 
    glUniform1i(loc_plant_clipMode, 0);
    glUniform1i(loc_plant_isReflectionPass, 0);
    glUseProgram(skyboxShader);
    glUniform1i(loc_skybox_isReflectionPass, 0);
    // =========================================================
    ...
    glUniformMatrix4fv(loc_shadow_lightSpaceMatrix, 1, GL_FALSE,
    ...
    glBindTexture(GL_TEXTURE_2D, soilNorm);
    glActiveTexture(GL_TEXTURE6);
    glBindTexture(GL_TEXTURE_2D, grassNorm);
    glActiveTexture(GL_TEXTURE7);
    glBindTexture(GL_TEXTURE_2D, mudRough);
    ...
    glUniformMatrix4fv(loc_terrain_projection, 1, GL_FALSE,
                       glm::value_ptr(projection));
    glUniformMatrix4fv(loc_terrain_lightSpaceMatrix, 1, GL_FALSE,
                       glm::value_ptr(lightSpaceMatrix));
    ...
    glUniformMatrix4fv(loc_plant_projection, 1, GL_FALSE,
                       glm::value_ptr(projection));
    glUniformMatrix4fv(loc_plant_lightSpaceMatrix, 1, GL_FALSE,
                       glm::value_ptr(lightSpaceMatrix));
    ...
    fishManager.render(fishShader, view, projection, lightSpaceMatrix, lightDir,
                       cameraPos, currentFrame, flashlightOn, cameraFront,
                       depthMap);
    ...
    glUniformMatrix4fv(loc_water_projection, 1, GL_FALSE,
                       glm::value_ptr(projection));
