...
void PlantManager::loadSpecies(const std::string& name, const std::string& maskPath, const std::string& tuftJsonPath, const std::string& texPath, const std::vector<std::string>& variantPaths, float scaleMult,
    ...
    // Potężne zwiększenie gęstości dla tataraku (użytkownik poprosił o dwukrotne zwiększenie, było 3mln, jest 6mln)
    ...
    for (int i = 0; i < numSpawnAttempts; ++i) {
        ...
        if (maskVal > 20) {
            ...
            // Tataraki rosną niesamowicie gęsto wszędzie tam gdzie sięga maska!
    ...
    for (size_t i = 0; i < species.variants.size(); ++i) {
        for (const auto& pair : variantChunkedMatrices[i]) {
            ...
            if (species.variants[i].hasFlatLOD) {
                ...
                // Używamy ZUPEŁNIE NOWEGO instVBO dla flat modeli, 
                // ponieważ flat mode może używać innej macierzy (np. bez rotacji X/Z w tuftsach)
...
void PlantManager::init() {
    ...
    // Tatarak — teraz posiada flat modele! (Wykorzystujemy zróżnicowane modele)
    loadSpecies("Tatarak", 
...
void PlantManager::render(unsigned int shader, const glm::vec3& camPos, const glm::mat4& vpMatrix) {
    ...
    // Rośliny to teraz pojedyncze quady (płaskie modele), więc możemy bezpiecznie drastycznie zwiększyć zasięg widoczności!
    // Pozwala to na pełne wtopienie wodorostów w mgłę (która osiąga 100% przy ~60-80m) zanim zostaną ucięte.
    const float RENDER_DIST = (camPos.y > 64.0f) ? 120.0f : 90.0f;
    ...
    for (auto& species : speciesList) {
        ...
        // Płynne przejście na 2m dla Tataraków, twarde cięcie na 12m dla wodorostów
        float currentLodThreshold = (species.name == "Tatarak") ? 2.0f : LOD_THRESHOLD;
        glUniform1f(loc_lodThreshold, currentLodThreshold);
        
        // Użytkownik chce by modele pojawiały się i znikały "dokładnie w miejscu", 
        // bez kropek / ditheringu / półprzezroczystości (twarde cięcie na granicy progu)
        bool smoothLOD = false; 
        ...
        for (auto& var : species.variants) {
            ...
            for (const auto& chunk : var.chunks) {
                ...
                if (useFlat && inFrustum) {
                    // lodFadeMode = 3 oznacza twarde cięcie dla FLAT modeli (potrzebne do innej obsługi alpha-testu)
